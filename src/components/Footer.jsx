...
export default function Footer() {
  return (
    <footer className="relative bg-gradient-to-b from-black via-zinc-900 to-black text-white px-6 md:px-16 pt-20 pb-8 overflow-hidden">
      ...
      {/* Yellow glow */}
      <div className="absolute -top-24 left-1/2 -translate-x-1/2 w-96 h-96 bg-yellow-400/20 blur-[140px] rounded-full" />
          ...
          <div className="flex gap-4 mt-6">
            {[FaFacebookF, FaInstagram, FaTwitter].map((Icon, i) => (
              <a
                key={i}
                href="#"
                className="w-10 h-10 rounded-full border border-yellow-400/30 flex items-center justify-center
                           text-yellow-400 hover:bg-yellow-400 hover:text-black transition-all duration-300"
              >
                <Icon />
              </a>
            ))}
